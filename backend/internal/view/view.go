...
func UserOut(user *store.User) User {
	return User{
		ID:        user.ID,
		Email:     user.Email,
...
func ConnectionOut(connection *store.Connection) Connection {
	return Connection{
...
func MessageOut(message *store.Message) Message {
	var override *string
...
func MessageBriefOut(message *store.Message) MessageBrief {
	return MessageBrief{
		ID:         message.ID,
		SenderName: message.SenderName,
		Subject:    message.Subject,
