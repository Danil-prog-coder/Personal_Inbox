...
// ErrUnavailable — модель не ответила или ответила не по схеме.
// Обрабатывается повтором (docs/00-product-spec.md, п. 6.1).
...
func responseSchema() map[string]any {
	return map[string]any{
		...
		"properties": map[string]any{
			"level":        map[string]any{"type": "string", "enum": store.Levels},
...
func BuildUserPrompt(request Request) string {
	...
	body := request.Body
	if len([]rune(body)) > config.LLMMaxBodyChars {
		body = string([]rune(body)[:config.LLMMaxBodyChars])
	}
	parts := []string{
...
// Analyze — один вызов модели на сообщение. Любой сбой — ErrUnavailable.
func (o *OpenAI) Analyze(request Request) (Result, error) {
	if o.APIKey == "" {
		return Result{}, fmt.Errorf("%w: OPENAI_API_KEY не задан", ErrUnavailable)
	}
	...
	if response.StatusCode != http.StatusOK {
		return Result{}, fmt.Errorf("%w: провайдер ответил %d", ErrUnavailable, response.StatusCode)
	}
	...
	if err := json.Unmarshal(raw, &completion); err != nil || len(completion.Choices) == 0 {
		return Result{}, fmt.Errorf("%w: ответ провайдера не разобрать", ErrUnavailable)
	}
...
func ParseResponse(raw string) (Result, error) {
	...
	if err := json.Unmarshal([]byte(raw), &data); err != nil {
		...
		return Result{}, fmt.Errorf("%w: ответ модели не разобрать: %s", ErrUnavailable, short)
	}

	level, _ := data["level"].(string)
	if !store.Contains(store.Levels, level) {
		return Result{}, fmt.Errorf("%w: неизвестный уровень: %v", ErrUnavailable, data["level"])
	}
	text := map[string]string{}
	for _, key := range []string{"category", "deadline", "summary"} {
		value, ok := data[key].(string)
		if !ok {
			return Result{}, fmt.Errorf("%w: поле %s не строка", ErrUnavailable, key)
		}
	...
	for _, key := range []string{"needs_reply", "needs_action"} {
		...
		if !ok {
			return Result{}, fmt.Errorf("%w: поле %s не булево", ErrUnavailable, key)
		}
		flags[key] = value
	}
	return Result{
