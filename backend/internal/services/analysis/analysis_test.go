...
func (f *fakeAnalyzer) Analyze(request openai.Request) (openai.Result, error) {
	f.calls++
	f.seen = request
	if f.calls <= f.failures {
		return openai.Result{}, fmt.Errorf("%w: тестовый сбой", exceptions.ErrModelUnavailable)
	}
	return f.result, nil
}
...
func TestUnavailableErrorIsRecognized(t *testing.T) {
	analyzer := &fakeAnalyzer{failures: 1, result: goodResult()}
	if _, err := analyzer.Analyze(openai.Request{}); !errors.Is(err, exceptions.ErrModelUnavailable) {
