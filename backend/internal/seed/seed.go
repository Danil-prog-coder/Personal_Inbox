...
func GetOrCreateUser(db *store.DB, passwordHash string) (*store.User, error) {
...
func GetOrCreateConnection(db *store.DB, user *store.User, kind string) (*store.Connection, error) {
...
func Seed(db *store.DB, passwordHash string, now time.Time) (int, error) {
	if now.IsZero() {
		now = store.UTCNow()
	}
...
func PlayLiveQueue(db *store.DB, bus *events.Bus, passwordHash string,
...
func playOne(db *store.DB, bus *events.Bus, passwordHash string, index int,
	...
	bus.Publish(user.ID, "message.created", view.MessageOut(message))
	...
	bus.Publish(user.ID, "message.analyzed", view.MessageOut(message))
	return nil
}
