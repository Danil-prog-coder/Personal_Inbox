...
func (s *Server) decodeSession(r *http.Request) session {
	...
	if time.Since(time.Unix(value.IssuedAt, 0)) > core.SessionMaxAge {
		return session{}
	}
	return value
}
...
func (s *Server) setSession(w http.ResponseWriter, value session) {
	http.SetCookie(w, &http.Cookie{
		...
		Path:     "/",
		MaxAge:   int(core.SessionMaxAge.Seconds()),
		HttpOnly: true,
...
func (s *Server) currentUser(w http.ResponseWriter, r *http.Request) (*sqlite.User, bool) {
