...
func NewWorker(db *store.DB, bus *events.Bus, analyzer llm.Analyzer) *Worker {
	return &Worker{
		db:          db,
		bus:         bus,
		analyzer:    analyzer,
		Sleep:       time.Sleep,
		RetryDelays: config.LLMRetryDelays,
		queue:       newQueue(),
...
func (w *Worker) BuildRequest(message *store.Message) (llm.Request, error) {
	...
	overrides, err := w.db.RecentOverrides(user.ID, config.LLMOverrideHistory)
	if err != nil {
		return llm.Request{}, err
	}
...
func (w *Worker) ProcessMessage(messageID int64) (*store.Message, error) {
	...
	w.bus.Publish(connection.UserID, "message.analyzed", view.MessageOut(message))
	return message, nil
}
...
// решение №15.
func QueueReanalysis(db *store.DB, enqueuer Enqueuer, userID int64) (int, error) {
