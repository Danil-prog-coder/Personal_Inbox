...
func (c *Client) checkCredentials() error {
	if c.ClientID == "" || c.ClientSecret == "" {
		return fmt.Errorf("%w: GOOGLE_CLIENT_ID и GOOGLE_CLIENT_SECRET не заданы", exceptions.ErrGmail)
	}
	return nil
}
...
func (c *Client) ExchangeCode(code string) (string, string, error) {
	...
	if err != nil {
		return "", "", fmt.Errorf("%w: Google отказал в обмене кода: %v", exceptions.ErrGmail, err)
	}
	if tokens.RefreshToken == "" {
		return "", "", fmt.Errorf(
			"%w: Google не выдал refresh_token — переподключите доступ", exceptions.ErrGmail)
	}
...
func (c *Client) accessToken(refreshToken string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("%w: обновление токена: %v", exceptions.ErrGmail, err)
	}
	if tokens.AccessToken == "" {
		return "", fmt.Errorf("%w: Google не выдал access_token", exceptions.ErrGmail)
	}
...
func (c *Client) get(accessToken, path string, params url.Values) (json.RawMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: %v", exceptions.ErrGmail, err)
	}
	...
	if response.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("%w: %d %s", exceptions.ErrGmail, response.StatusCode,
...
func (c *Client) profileEmail(accessToken string) (string, error) {
	...
	if err := json.Unmarshal(raw, &profile); err != nil {
		return "", fmt.Errorf("%w: профиль не разобрать", exceptions.ErrGmail)
	}
...
func (c *Client) initialIDs(accessToken string) ([]string, error) {
	after := time.Now().UTC().AddDate(0, 0, -core.GmailImportDays).Format("2006/01/02")
	var ids []string
	pageToken := ""
	for len(ids) < core.GmailImportLimit {
		limit := core.GmailImportLimit - len(ids)
		if limit > pageSize {
			limit = pageSize
		}
		params := url.Values{
		...
		if err := json.Unmarshal(raw, &page); err != nil {
			return nil, fmt.Errorf("%w: список писем не разобрать", exceptions.ErrGmail)
		}
	...
	if len(ids) > core.GmailImportLimit {
		ids = ids[:core.GmailImportLimit]
	}
	return ids, nil
}
...
func (c *Client) incrementalIDs(accessToken, startHistoryID string) ([]string, string, error) {
	...
	for {
		...
		if err := json.Unmarshal(raw, &page); err != nil {
			return nil, "", fmt.Errorf("%w: историю не разобрать", exceptions.ErrGmail)
		}
