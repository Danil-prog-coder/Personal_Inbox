...
type messageModel = store.Message
...
func (e *env) serverConfig(secret string) config.Config {
	return config.Config{SessionSecret: secret, FrontendOrigin: "http://localhost:5173"}
}

// user создаёт пользователя прямо в базе, минуя регистрацию.
func (e *env) user(email string) *store.User {
	e.t.Helper()
...
func (e *env) authorized() *store.User {
	e.t.Helper()
...
func (e *env) connection(user *store.User, kind, state string) *store.Connection {
	e.t.Helper()
...
func (e *env) message(connection *store.Connection, apply func(*store.Message)) *store.Message {
	e.t.Helper()
	e.counter++
	message := &store.Message{
		...
		ReceivedAt:   store.UTCNow().Add(-time.Duration(e.counter) * time.Minute),
		Status:       "DONE",
		Level:        "NORMAL",
		Category:     "Работа",
