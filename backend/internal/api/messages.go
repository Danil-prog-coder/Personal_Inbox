...
func param(w http.ResponseWriter, r *http.Request, name, fallback string, allowed []string) (string, bool) {
	...
	if !store.Contains(allowed, value) {
...
func (s *Server) ownedMessage(w http.ResponseWriter, r *http.Request, user *store.User) (*store.Message, bool) {
...
func (s *Server) handleListMessages(w http.ResponseWriter, r *http.Request) {
	...
	source, ok := param(w, r, "source", "", store.SourceKinds)
	if !ok {
		return
	}
	...
	messages, err := s.db.FilteredMessages(user.ID, store.Filter{
		Source:   source,
		Level:    level,
		Status:   status,
		Reply:    reply,
	...
	items := make([]view.Message, 0, len(messages))
	unread := 0
	for _, message := range messages {
		items = append(items, view.MessageOut(message))
		if !message.IsRead {
			unread++
		}
	}
	respond(w, http.StatusOK, view.MessageList{
		Items:  items,
		Total:  len(items),
		Unread: unread,
	})
...
func (s *Server) handleGetMessage(w http.ResponseWriter, r *http.Request) {
	...
	respond(w, http.StatusOK, view.MessageOut(message))
}
...
func (s *Server) handleSetLevel(w http.ResponseWriter, r *http.Request) {
	...
	if !store.Contains(store.Levels, payload.Level) {
	...
	s.bus.Publish(user.ID, "message.analyzed", view.MessageOut(message))
	respond(w, http.StatusOK, view.MessageOut(message))
}
