...
func TestListReturnsNewestFirst(t *testing.T) {
	...
	older := e.message(connection, func(m *messageModel) {
		m.Subject = "Старое"
		m.ReceivedAt = store.UTCNow().Add(-2 * time.Hour)
	})
	newer := e.message(connection, func(m *messageModel) {
		m.Subject = "Новое"
		m.ReceivedAt = store.UTCNow()
	})
...
func TestFilterByPeriod(t *testing.T) {
	...
	user := e.authorized()
	connection := e.connection(user, "gmail", "active")
	e.message(connection, func(m *messageModel) { m.ReceivedAt = store.UTCNow().Add(-time.Hour) })
	e.message(connection, func(m *messageModel) {
		m.ReceivedAt = store.UTCNow().Add(-40 * 24 * time.Hour)
	})
