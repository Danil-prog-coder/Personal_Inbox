...
func newIngestor(t *testing.T) (*ingest.Ingestor, *store.Connection) {
	t.Helper()
...
func TestImportLimitIsRespected(t *testing.T) {
	if config.GmailImportLimit != 200 || config.GmailImportDays != 30 {
		t.Fatalf("границы первого импорта изменены: %d писем за %d дней",
			config.GmailImportLimit, config.GmailImportDays)
	}
}
