...
// ErrTelegram — Bot API вернул ошибку или недоступен.
...
func (c *Client) Call(token, method string, params map[string]any) (json.RawMessage, error) {
	...
	if !payload.OK {
		...
		return nil, fmt.Errorf("%w: %s", ErrTelegram, description)
	}
...
func (c *Client) VerifyToken(token string) (string, error) {
	...
	if err := json.Unmarshal(raw, &bot); err != nil {
		return "", fmt.Errorf("%w: ответ getMe не разобрать", ErrTelegram)
	}
	if bot.Username != "" {
