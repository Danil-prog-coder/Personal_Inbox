...
func TestInvalidResponseIsUnavailable(t *testing.T) {
	...
	for name, raw := range cases {
		if _, err := ParseResponse(raw); !errors.Is(err, exceptions.ErrModelUnavailable) {
...
func TestPromptTruncatesLongBody(t *testing.T) {
	body := strings.Repeat("я", core.LLMMaxBodyChars+500)
	prompt := BuildUserPrompt(Request{Body: body})
	if !strings.Contains(prompt, strings.Repeat("я", core.LLMMaxBodyChars)) {
		t.Fatal("текст обрезан раньше границы в 4000 символов")
	}
	if strings.Contains(prompt, strings.Repeat("я", core.LLMMaxBodyChars+1)) {
...
func TestAnalyzeWithoutAPIKeyIsUnavailable(t *testing.T) {
	...
	if _, err := client.Analyze(Request{}); !errors.Is(err, exceptions.ErrModelUnavailable) {
